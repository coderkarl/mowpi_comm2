...
class MicroSerial():
    ...
    def __init__(self):
        ...
        self.serial = serial.Serial('/dev/ttyACM0', baudrate=115200, timeout=0.01) #CHANGE TIMEOUT TO SHORT HERE
...
class Controller():
    ...
    def write_speed_curv(self, speed_in, curv_in):
        ...
        self.curv = curv_in # steer angle deg
        #print('speed, curv', speed_in, curv_in)
        
        raw_speed = int(speed_in*10.0)+120
        raw_omega = int(curv_in)+120
...
class MicroBridge(Node):
    def __init__(self):
        super().__init__("descent_ros_comm")
        ...
        self.get_logger().info("Started Descent Serial ROS Comm. V2 V2")
    ...
    def drive_callback(self, data):
        v = data.linear.x
        steer_deg = -data.angular.z * 180.0 / pi
        max_speed = 1.2
        max_steer_deg = 40
        ...
        if steer_deg > max_steer_deg:
            steer_deg = max_steer_deg
        elif steer_deg < -max_steer_deg:
            steer_deg = -max_steer_deg
